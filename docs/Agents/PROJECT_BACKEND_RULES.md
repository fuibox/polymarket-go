...
### 2.3 Polymarket API 实际行为与文档不符的已知案例
文档不完全可信，以下行为经实测确认，与官方文档描述不一致：

| 接口 | 文档说明 | 实际行为 | 处理方式 |
|---|---|---|---|
| `GET /data/order/{id}` | 订单不存在返回 404 | 实际返回 200 + 空 body（`{}`） | 检查 `result.ID == ""` 作为不存在的判断依据 |

遇到新的文档与实际不符的情况，必须在此处补充记录。
...
## 10) CLOB v2 迁移期规则（2026-04-28 切换）

v1 与 v2 交易链路并行存在，由 `ClobClient.protocolVersion` 派发。

### 10.1 新代码的 v2 字段约束
- `timestamp` 是 **毫秒**（`time.Now().UnixMilli()`），不是秒。写错单位后端直接拒单。
- `metadata` / `builder` 必须是 0x-prefixed 32-byte hex（64 个 hex 字符），**不补零**。传入短串或奇数长度签名器会报错拒绝——这是有意的，避免短值被静默补零导致上链归因错位。
- `feeRateBps`、`nonce`、`expiration`、`taker` 在 v2 签名 Order 里**不存在**。若给 v2 path 传这些 `OrderArgs` 字段，会被静默忽略——不要依赖它们。

### 10.2 v2 Domain
- `name = "Polymarket CTF Exchange"`、`version = "2"`、`chainId`、`verifyingContract = ContractConfig.ExchangeV2`（或 NegExchangeV2）。
- 新代码不要再硬编码 v1 地址 / version=1；v2 path 必须走 `ContractConfig.ExchangeV2` / `NegExchangeV2`。

### 10.3 ABI 未验证项目
`client/relayer/client_v2.go` 内 `wrap(uint256)` / `unwrap(uint256)` 函数签名是根据 v2 文档措辞的保守假设，**未与链上字节码对照过**。主网上 wrap / unwrap 前必须验证；验证后请更新 `wrapFunctionSig` / `unwrapFunctionSig` 常量并删除本条提醒。

### 10.4 Amoy v2 尚未公开
Polymarket 未公开 Amoy 的 v2 Exchange / NegExchange / pUSD / CollateralOnramp 地址。`ContractConfig` 对 chain 80002 这几项是零地址，v2-on-Amoy 调用会因 `exchange address is zero` 失败——这是有意的，避免静默走到 v1 合约。拿到地址后补到 `client/config/config.go`。

### 10.5 v2 POST /order 与 Cancel 响应
目前假定响应体结构与 v1 `types.OrderResponse` 同构，Cancel 接口路径与 body 格式与 v1 一致。testnet 首次联调必须验证；若不一致再分叉。**不要**预先分叉。

### 10.6 Builder HMAC 保留
`tools/headers/headers.go` 的 `L2WithBuilderHeader` / `BuilderConfig` / `GenerateBuilderHeaders` / `InsertBuilderHeaders` 在 v2 仍保留（后端忽略但无害）。4/28 之后统一清理，不要在迁移期随手删。

### 10.7 4/28 后删除清单
切换当日的独立 PR 统一删除：v1 `utils_order_builder` 整包、`OrderArgs` 的 v1-only 字段、Builder HMAC 相关代码、`ResolveFeeRateBps` / `GetFeeRateBps`、`ContractConfig.Exchange` / `NegExchange` 的 v1 地址、`ProtocolVersion` 字段本身。
